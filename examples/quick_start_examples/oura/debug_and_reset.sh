# debug and reset for multiple tables but need to update the configuration.json references - not tested yet
...
# Extract all table names (POSIX-compatible)
TABLE_NAMES=($(grep '"table"' "$CONNECTOR_FILE" | sed -E 's/.*"table": *"([^"]+)".*/\1/' | sort | uniq))

if [ ${#TABLE_NAMES[@]} -eq 0 ]; then
    echo -e "${YELLOW}Warning: No tables found in connector.py. Using fallback 'fpr_records'.${NC}"
    TABLE_NAMES=("fpr_records")
else
    echo -e "${GREEN}✓ Detected table(s): ${TABLE_NAMES[*]}${NC}"
fi
...
if [ -f "$WAREHOUSE_DB" ]; then
    if command -v duckdb >/dev/null 2>&1; then
        ...
        echo -e "${YELLOW}DuckDB CLI not found. Manually run the following:${NC}"
        for TABLE_NAME in "${TABLE_NAMES[@]}"; do
            echo "duckdb \"$WAREHOUSE_DB\" \"SELECT * FROM tester.${TABLE_NAME} LIMIT 5;\""
        done
    fi
else
    echo -e "${YELLOW}warehouse.db not found. Searching for alternatives...${NC}"
