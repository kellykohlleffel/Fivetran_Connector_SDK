...
def schema(configuration: dict):
    return [
        ...
        {
            "table": "zip_codes",  # Name of the table for zip code metadata
...
def get_coordinates_from_zip(zip_code: str) -> tuple:
    ...
    coordinates = data['places'][0]  # Get the first place in the zip code
    lat = float(coordinates['latitude'])
    lon = float(coordinates['longitude'])
    coordinates["zip_code"] = zip_code
    
    log.info(f"Found coordinates: ({lat}, {lon})")
    return (lat, lon), coordinates  # Return both coordinates and metadata
...
def update(configuration: dict, state: dict):
    ...
    zip_codes_str = configuration.get('zip_codes', '95113')  # Default to downtown San Jose
    ...
    for zip_code in zip_codes:
        try:
            ...
            yield op.upsert(table="zip_codes", data=metadata)
            ...
            for period in periods:
                ...
                period['zip_code'] = zip_code

                # This log message will only show while debugging.
                log.fine(f"period={period['name']} for zip code {zip_code}")

                # Yield an upsert operation to insert/update the row in the "period" table.
                yield op.upsert(table="forecast", data=period)
